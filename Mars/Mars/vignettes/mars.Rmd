...
*Mars* is a non-parametric regression technique that automatically models nonlinearities and interactions between variables, acting as an extension of linear models.

#### Details

The Multivariate Adaptive Regression Spline (MARS) is a type of regression analysis introduced by Jerome H. Friedman in 1991. 
MARS begins by fitting a simple linear regression model to the data. The predictor variable that has the highest correlation with the response variable is selected to split the data into two subsets. The algorithm then fits a linear regression model to each subset, using the selected predictor variable. The residuals from these two models are used to construct a new predictor variable, which is added to the model. This process is repeated until a stopping criterion is met (e.g. the maximum number of basis functions is reached, or the improvement in the loss function is below a specified threshold). The resulting model consists of a series of piecewise linear segments, each with its own slope and intercept.
...
C'(M) is the sum of the hat-values from the fitted model and the smoothing parameter d times M, where d is typically set to 3, according to Friedman.
...
an object of class 'mars' that contains the value: fitted value and coefficients.
...
Jerome H. Friedman, The Annals of Statistics, Mar., 1991, Vol. 19, No. 1 (Mar., 1991), pp. 1-67. <https://www.jstor.org/stable/2241837%7D.>
